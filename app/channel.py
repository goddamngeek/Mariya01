...
# Нажатие, пришедшее из веб-чата, а не из телеграма. Подтверждать его
# нечем: кружок ожидания на кнопке рисует телеграмный клиент, а страница
# гасит свою кнопку сама.
WEB_PRESS = "web"


async def answer_callback_query(callback_query_id: str, text: str | None = None) -> bool:
    """Ответ на нажатие — единственная функция без адреса: у нажатия есть
    только собственный идентификатор, и транспорт в нём подразумевается.
    Пока их два, различаем по сторожевому значению; станет больше — придётся
    передавать транспорт явно."""
    if callback_query_id == WEB_PRESS:
        return True
