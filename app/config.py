import hashlib
import os
from datetime import time
from zoneinfo import ZoneInfo

TIMEZONE = ZoneInfo("Europe/Moscow")

WATER_REMINDER_WINDOWS = [
    (time(10, 0), time(12, 0)),
    (time(13, 0), time(15, 0)),
    (time(16, 0), time(18, 0)),
    (time(19, 0), time(22, 0)),
]
WATER_REMINDER_TEXTS = [
    "Эй, а водичку сегодня пил?",
    "Стоп. Вода. Сейчас.",
    "Мозг работает на 80% воды — пополни запасы.",
    "Кофе не считается. Налей воды.",
    "Экран подождёт, стакан воды — нет.",
    "Твой организм просит стакан H₂O.",
    "Маленькая пауза: вода, потом продолжаем.",
    "Если читаешь это — значит, пора пить.",
    "Вода — самый простой апгрейд твоего дня.",
    "Чашка чая — это не вода, напоминаю.",
    "Сделай глоток, потом дочитаешь.",
    "Уровень воды в организме: требует пополнения.",
    "Три глотка воды — и снова в бой.",
    "Не жди жажды, пей заранее.",
    "Вода рядом? Тогда самое время.",
]

MAX_REGISTERED_USERS = 2

# Публичный канал с «мыслью дня» из «Круга чтения» (see app/parables.py).
# Не секрет — публичный юзернейм, поэтому здесь, а не в переменных окружения.
# Пустая строка выключает публикацию.
THOUGHT_CHANNEL = "@blessandbeblessed"

DATABASE_URL = os.environ.get(
    "DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/odysseus_queue_bot"
)
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "")
SYNC_BEARER_TOKEN = os.environ.get("SYNC_BEARER_TOKEN", "")


# Direct Trilium ETAPI access — used for the fully-deterministic writes
# (ежедневник) that don't need Odysseus/an LLM at all, so they don't have
# to pay for Odysseus's whole session/auth stack just to reach Trilium.
# Same instance and token Odysseus itself uses, reached over its own public
# Caddy-proxied subdomain rather than the internal-only docker network.
TRILIUM_URL = os.environ.get("TRILIUM_URL", "").rstrip("/")
TRILIUM_ETAPI_TOKEN = os.environ.get("TRILIUM_ETAPI_TOKEN", "")

# Траты — beancount вместо Firefly (см. app/ledger.py).
#
# Здесь, а не в переменных окружения: это не секрет, а раскладка счетов, и
# читать её надо рядом с кодом, который её использует.
#
# Счета РАЗДЕЛЕНЫ по людям, категории ОБЩИЕ. Личные файлы складываются в
# общий одним include, и там карты двоих обязаны остаться разными счетами, а
# еда — наоборот, должна сложиться в одну сумму на семью.
#
# Корни счетов — английские, всё остальное по-русски. Кириллические корни
# beancount умеет (option "name_assets"), но переименование НЕ переживает
# include: подключаемый файл разбирается со стандартными корнями, и
# «Активы:…» в нём невалидны всегда (проверено на 3.2.3). А выгрузка бота —
# именно подключаемый файл. Пять английских слов выучиваются один раз.
LEDGER_CURRENCY = "RUB"

LEDGER_ACCOUNTS = {
    # Перенесено из Firefly перед его сносом 7 сентября 2026 — это реальные
    # счета Остапа, а не выдуманные мной при первом заходе.
    #
    # Кредитка, рассрочка и Халва — Liabilities, а не Assets: это не твои
    # деньги, а долг, и в семейном виде они не должны складываться с
    # накоплениями. Для бота разницы нет — вторая нога проводки считается
    # одинаково, — но арифметика в отчётах становится честной.
    "ОСТАП": [
        "Assets:Остап:Озон-Карта",
        "Assets:Остап:Наличные",
        "Assets:Остап:Озон-Накопительный",
        "Assets:Остап:ДомРФ",
        "Assets:Остап:МТС-Брокер",
        "Liabilities:Остап:Озон-Кредитка",
        "Liabilities:Остап:Озон-Рассрочка",
        "Liabilities:Остап:Халва",
    ],
    # У Маши в Firefly счетов не было вообще — токен ей так и не завели.
    # Заведёт свои, когда дойдут руки.
    "МАША": [
        "Assets:Маша:Озон",
        "Assets:Маша:Наличные",
    ],
}

LEDGER_CATEGORIES = [
    "Expenses:Еда",
    "Expenses:Дом",
    "Expenses:Транспорт",
    "Expenses:Здоровье",
    "Expenses:Развлечения",
    "Expenses:Подписки",
    "Expenses:Одежда",
    "Expenses:Прочее",
]

# Куда падает трата, если категорию не выбрали. Не спрашивать про неё сразу —
# осознанное решение: записать покупку можно только сейчас, а разложить по
# полкам — когда угодно потом.
LEDGER_DEFAULT_CATEGORY = "Expenses:Прочее"


def ledger_label(account: str) -> str:
    """Кнопка показывает последнее звено: «Озон», а не «Активы:Остап:Озон»."""
    return account.rsplit(":", 1)[-1]


# Веб-чат — второй вход в бота на случай, когда телеграм недоступен (см.
# app/web.py). Токен у каждого человека свой: он же определяет, ЧЕЙ разговор
# показывать, — общего представления быть не должно.
#
# Токен приходит один раз в адресе (?t=…), обменивается на куку и из адреса
# исчезает: в истории браузера и логах он иначе оставался бы навсегда.
# Пустое значение выключает вход этого человека, пустые оба — весь веб-чат.
WEB_TOKENS = {
    "ОСТАП": os.environ.get("WEB_TOKEN_OSTAP", ""),
    "МАША": os.environ.get("WEB_TOKEN_MASHA", ""),
}


# Телеграм подписывает каждый запрос к вебхуку этим значением (заголовок
# X-Telegram-Bot-Api-Secret-Token), если передать его в setWebhook. Без
# проверки вебхук принимает ЛЮБОЙ POST: адрес бота не секрет, а
# is_registered() смотрит на chat_id ИЗ САМОГО ЗАПРОСА — то есть подделать
# сообщение от любого из двоих мог кто угодно.
#
# Значение по умолчанию выводится из токена бота, а не требует отдельной
# переменной: так защита включается сразу на деплое, без окна, в котором
# переменную ещё не проставили. Знать его может только тот, кто и так знает
# токен бота, то есть владеет ботом целиком.
TELEGRAM_WEBHOOK_SECRET = os.environ.get("TELEGRAM_WEBHOOK_SECRET") or (
    hashlib.sha256(f"webhook:{TELEGRAM_BOT_TOKEN}".encode()).hexdigest()
    if TELEGRAM_BOT_TOKEN else ""
)
